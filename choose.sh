...
bash name.sh "$name" 0
	...
	echo "1]Hall7    8]Hall8    9]Hall9    10]Hall10    11]Hall11    12]Hall12"
	read -p "" hall
	bash hall.sh "$hall" 0
		
elif [[ $code == "3" ]]; then
	echo "Choose Batch"
	echo "1]Y17    2]Y16    3]Y15    4]Y14    5]Y13    6]Y12"
	read -p "" batch
	bash batch.sh "$batch" 0	

elif [[ $code == "4" ]]; then 
	echo "Type in the username"
	read -p "" username	
	bash username.sh "$username" 0
	...
	read -p "" degree
	bash program.sh "$degree" 0
		
elif [[ $code == "7" ]]; then
 	echo "Type in the roll no"
	read -p "" input_roll
	bash roll_no.sh "$input_roll" 0
		
else
	echo "Choose Gender(M/F)"
	read -p "" gender
	bash gender.sh "$gender" 0	
fi